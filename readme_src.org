...
| =MOPACPot= | =MOPAC= | Frontend always builds; load =libmopacc= from the split [[https://github.com/OmniPotentRPC/mopacc][mopacc]] project at runtime (OpenMOPAC). Packed params. Default model AM1. |
