...
| =MOPACPot= | =MOPAC= | Frontend always builds; load =libmopacc= from the split [[https://github.com/OmniPotentRPC/mopacc][mopacc]] project at runtime (OpenMOPAC). Packed params, not Cap'n Proto. Default model AM1. Not the AMS MOPAC engine |
