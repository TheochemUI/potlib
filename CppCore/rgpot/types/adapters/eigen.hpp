...
inline Eigen::MatrixXd convertToEigen(const AtomMatrix& atomMatrix) {
    return Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor>>(atomMatrix.data(), atomMatrix.rows(), atomMatrix.cols());
}
...
inline std::array<std::array<double, 3>, 3> convertToEigen3d(const Eigen::Matrix3d& matrix) {
